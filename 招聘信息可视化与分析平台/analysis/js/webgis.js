    ...
    var map = new ol.Map({
        ...
        layers:[baidu],
        view: new ol.View({
            center: ol.proj.fromLonLat([116.395645038,39.9299857781]),
            zoom: 5.8
        })
    });
    ...
    function creat_cqlurl(option1,option2,option3,option4,option5){
        var featureRequest = new ol.format.WFS().writeGetFeature({
            ...
            filter:new ol.format.filter.and(ol.format.filter.equalTo('class2',option2 ),
             ...
             ol.format.filter.and(ol.format.filter.greaterThan("max_salary",parseFloat(option5)),
             ol.format.filter.lessThan("min_salary",parseFloat(option4)))
             )
          }); 
        ...
        for(i=0;i<options1.length;i++){
            for(n=0;n<options2.length;n++){
                ...
                if(shengfeng_s!='all'&&leibie_s!='all'){
                    creat_cqlurl(shengfeng_s,leibie_s,option3,option5,option4);
                }
            }
        }  
    }
